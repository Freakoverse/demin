...
const tabBar = {
  ...
  updateTab: function (tabId, tabEl = tabBar.getTab(tabId)) {
    ...
    // Handle security icons
    var insecureIcon = tabEl.getElementsByClassName('icon-tab-not-secure')[0]
    var secureIcon = tabEl.getElementsByClassName('icon-tab-secure')[0]

    // V2 DNN hostname detection using full BIP39 wordlist
    const bip39WordsTab = require('../util/bip39words.js')
    function matchBIP39WordTab(str) {
      const maxLen = Math.min(8, str.length)
      for (let len = maxLen; len >= 3; len--) {
        if (bip39WordsTab.has(str.slice(0, len))) return str.slice(0, len)
      }
      return null
    }
    function isDNNHostname(hostname) {
      ...
      const w1 = matchBIP39WordTab(rest)
      if (!w1) return false
      rest = rest.slice(w1.length)
      const w2 = matchBIP39WordTab(rest)
      if (!w2) return false
      rest = rest.slice(w2.length)
      return rest && /^\d*[a-z]+$/.test(rest)
    }

    // Check if this is a DNN URL by hostname pattern
    let isDnnUrl = false
    try {
      const urlObj = new URL(tabData.url || '')
      isDnnUrl = isDNNHostname(urlObj.hostname)
    } catch (e) { }

    if (isDnnUrl) {
      // DNN URLs: show locked icon for verified, unlocked for not verified
      if (tabData.secure === true) {
        // Show locked icon, remove unlocked
        if (insecureIcon) insecureIcon.remove()
        if (!secureIcon) {
          var secureIcon = document.createElement('i')
          secureIcon.className = 'icon-tab-secure tab-icon tab-info-icon i carbon:locked'
          secureIcon.title = 'DNN Certificate Verified'
          secureIcon.style.color = '#4ade80' // Green color
          iconArea.insertBefore(secureIcon, iconArea.firstChild)
        }
      } else {
        // Show unlocked icon, remove locked
        if (secureIcon) secureIcon.remove()
        if (!insecureIcon) {
          ...
          insecureIcon.title = 'DNN Certificate Not Verified'
          insecureIcon.style.color = '#f87171' // Red color
          iconArea.insertBefore(insecureIcon, iconArea.firstChild)
        }
      }
    } else {
      // Non-DNN URLs: original behavior (only show unlocked for insecure)
      if (secureIcon) secureIcon.remove() // Remove DNN icon if switching from DNN URL
