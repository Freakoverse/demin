...
const dnnSeedNodesList = document.getElementById('dnn-seed-nodes-list')
const dnnCustomNodesList = document.getElementById('dnn-custom-nodes-list')
const dnnPoolNodesList = document.getElementById('dnn-pool-nodes-list')
const dnnNewNodeInput = document.getElementById('dnn-new-node-input')
const dnnAddNodeBtn = document.getElementById('dnn-add-node-btn')
const dnnRefreshPoolBtn = document.getElementById('dnn-refresh-pool-btn')
const dnnPoolStatus = document.getElementById('dnn-pool-status')

// Display seed nodes (read-only)
DNN_SEED_NODES.forEach(function (url) {
  const li = document.createElement('li')
  const urlSpan = document.createElement('span')
  urlSpan.className = 'node-url'
  urlSpan.textContent = url
  li.appendChild(urlSpan)
  dnnSeedNodesList.appendChild(li)
})

// Create a custom node list item with delete button
function createCustomNodeItem(url) {
  const li = document.createElement('li')
  ...
  deleteBtn.className = 'node-delete i carbon:close'
  deleteBtn.title = 'Remove'
  deleteBtn.addEventListener('click', function () {
    li.remove()
    settings.get('dnnCustomNodes', function (nodes) {
      if (!nodes) nodes = []
      const filtered = nodes.filter(n => n !== url)
      settings.set('dnnCustomNodes', filtered)
      postMessage({ message: 'dnnCustomNodesChanged', nodes: filtered })
    })
  })
...
// Load and display custom nodes
settings.get('dnnCustomNodes', function (nodes) {
  if (nodes && nodes.length > 0) {
    nodes.forEach(function (url) {
      dnnCustomNodesList.appendChild(createCustomNodeItem(url))
    })
  }
})

// Add new custom node
dnnAddNodeBtn.addEventListener('click', function () {
  const url = dnnNewNodeInput.value.trim()
  if (!url) return
  ...
  try {
    ...
    alert('Please enter a valid URL (e.g., https://example.com:8080)')
    return
  }
  ...
  settings.get('dnnCustomNodes', function (nodes) {
    ...
    if (nodes.includes(url) || DNN_SEED_NODES.includes(url)) {
      alert('This node is already in the list')
      return
    }

    nodes.push(url)
    settings.set('dnnCustomNodes', nodes)
    dnnCustomNodesList.appendChild(createCustomNodeItem(url))
    dnnNewNodeInput.value = ''
    postMessage({ message: 'dnnCustomNodesChanged', nodes: nodes })
  })
})

// Allow Enter key to add node
dnnNewNodeInput.addEventListener('keypress', function (e) {
  if (e.key === 'Enter') {
    dnnAddNodeBtn.click()
  }
})

// Create a pool node item with status
function createPoolNodeItem(url, status) {
  const li = document.createElement('li')
  ...
  statusSpan.className = 'node-status ' + (status === 'healthy' ? 'healthy' : 'unknown')
...
// Request pool data from main process
function refreshPoolDisplay() {
  dnnPoolStatus.textContent = 'Loading...'
  postMessage({ message: 'getDNNNodePool' })
}

// Handle pool data response
window.addEventListener('message', function (e) {
  if (e.data && e.data.message === 'receiveDNNNodePool') {
    dnnPoolNodesList.innerHTML = ''
    const nodes = e.data.nodes || []

    if (nodes.length === 0) {
      dnnPoolStatus.textContent = 'No nodes discovered yet. Click "Refresh Pool" to start discovery.'
    } else {
      dnnPoolStatus.textContent = nodes.length + ' healthy node(s) discovered'
      nodes.forEach(function (node) {
        dnnPoolNodesList.appendChild(createPoolNodeItem(node.url, node.status))
      })
    }
  }
})
...
// Refresh pool button
dnnRefreshPoolBtn.addEventListener('click', function () {
  dnnPoolStatus.textContent = 'Refreshing...'
  postMessage({ message: 'refreshDNNNodePool' })
  // Wait a bit then request updated pool
  setTimeout(refreshPoolDisplay, 2000)
})
