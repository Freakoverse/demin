...
function loadURLInView(id, url, win) {
  // Check if this is a DNN URL that needs interception
  if (global.isDNNHostname) {
    try {
      const urlObj = new URL(url)
      const hostname = urlObj.hostname

      if (global.isDNNHostname(hostname)) {
        console.log(`[DNN ViewManager] Intercepting DNN URL: ${url}`)

        // Check for proceed flag
        if (urlObj.searchParams.get('__dnn_proceed') === 'true') {
          console.log(`[DNN ViewManager] User chose to proceed for: ${hostname}`)
          global.proceedAnywayDomains.add(hostname.toLowerCase())
          urlObj.searchParams.delete('__dnn_proceed')
          url = urlObj.toString()
        }

        // Resolve DNN and redirect
        global.resolveDNN(hostname).then(resolution => {
          if (resolution) {
            // Check if cert is verified OR user chose to proceed
            if (!resolution.certVerified && !global.proceedAnywayDomains.has(hostname.toLowerCase())) {
              // Show warning page
              console.log(`[DNN ViewManager] Cert not verified for ${hostname}, showing warning page`)
              ...
              viewMap[id].webContents.loadURL(warningUrl)
              viewStateMap[id].loadedInitialURL = true
            ...
            // Redirect to resolved IP
            const redirectURL = `https://${resolution.ip}:${resolution.port}${urlObj.pathname}${urlObj.search}`
            console.log(`[DNN ViewManager] Redirecting to: ${redirectURL}`)

            // Send tab-to-DNN mapping to renderer for URL display
            const eventTarget = getWindowFromViewContents(viewMap[id].webContents) || windows.getCurrent()
            if (eventTarget) {
              getWindowWebContents(eventTarget).send('set-tab-dnn-mapping', {
                tabId: id,
                dnnName: hostname.toLowerCase(),
                originalUrl: url
              })
            }

            // Send cert status to renderer
            if (global.sendDNNCertStatus) {
              global.sendDNNCertStatus(hostname, resolution)
            }
            ...
            viewMap[id].webContents.loadURL(redirectURL)
            viewStateMap[id].loadedInitialURL = true
          } else {
            console.log(`[DNN ViewManager] Could not resolve: ${hostname}`)
            // Fall through to normal loading (will fail but show error)
            doNormalLoad()
          }
        }).catch(e => {
          console.error('[DNN ViewManager] Resolution error:', e)
          doNormalLoad()
        })
        return // Don't do normal load - async handler will do it
      }
    } catch (e) {
      // URL parse error, fall through to normal load
    }
  }

  doNormalLoad()

  function doNormalLoad() {
