...
// Debug: Log IPC messages from renderer about DNN cert verification
ipc.on('dnn-ipc-received', function (e, data) {
  console.log('[DNN IPC Response from Renderer]:', JSON.stringify(data))
})
...
/* DNN Node Pool IPC handlers for settings */

// dnnNodeDiscovery is a global variable from the concatenated modules

ipc.on('getDNNNodePool', function (e) {
  const nodes = dnnNodeDiscovery.getPoolStatus()
  e.sender.send('receiveDNNNodePool', nodes)
})

ipc.on('refreshDNNNodePool', function (e) {
  dnnNodeDiscovery.refreshPool().then(() => {
    const nodes = dnnNodeDiscovery.getPoolStatus()
    e.sender.send('receiveDNNNodePool', nodes)
  })
})

ipc.on('dnnCustomNodesChanged', function (e, nodes) {
  dnnNodeDiscovery.setCustomNodes(nodes)
})

// Load custom nodes from settings on startup
app.on('ready', function () {
  const customNodes = settings.get('dnnCustomNodes')
  if (customNodes && Array.isArray(customNodes)) {
    dnnNodeDiscovery.setCustomNodes(customNodes)
  }
})
