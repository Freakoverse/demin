...
async function afterPackageBuilt(packagePath) {
  ...
  /* create zip files */
  var output = fs.createWriteStream('dist/app/' + 'Demin-v' + version + '-windows' + archSuffix + '.zip')
  var archive = archiver('zip', {
    zlib: { level: 9 }
  })
  archive.directory(packagePath, 'Demin-v' + version)
  archive.pipe(output)
  ...
  const options = {
    src: packagePath,
    dest: 'dist/app/demin-installer' + archSuffix,
    icon: 'icons/icon256.ico',
    animation: 'icons/windows-installer.gif',
    licenseUrl: 'https://github.com/AustinKelsworthy/Amber/blob/master/LICENSE.txt',
    noMsi: true
  }
  ...
  await installer(options)
    .then(function () {
      fs.renameSync('./dist/app/demin-installer' + archSuffix + '/demin-' + version + '-setup.exe', './dist/app/demin-' + version + archSuffix + '-setup.exe')
    })
    .catch(err => {
